...
#include "Math.hpp"
#include "Int.hpp" // isPowerOfTwo, HUGE_PRIME_0
#include <vector>
#include <ostream>
#include <cassert>
...
namespace math
{
	...
	public:
		...
		{
			if (isZero(math::abs(vec.x) - math::abs(vec.y)))
			{
				// N * 45 deg
				vec.x = signF(vec.x);
				vec.y = signF(vec.y);
	...
	inline bool isPowerOfTwo(Vec2u vec) {
		return math::isPowerOfTwo(vec.x)
		&&
		math::isPowerOfTwo(vec.y);
	}
	...
	inline Vec2T<T> clamp(Vec2T<T> v, Vec2T<T> mn, Vec2T<T> mx) {
		Vec2T<T> ret;
		for (int d=0; d<2; ++d)
			ret[d] = math::clamp(v[d], mn[d], mx[d]);
		return ret;
	}
...
namespace std
{
	template<typename T> struct hash<math::Vec2T<T>> {
		size_t operator()(const math::Vec2T<T>& val) const {
			return std::hash<T>()(val.x) + std::hash<T>()(val.y) * math::HUGE_PRIME_0;
		}
	};
}
